...
local function match_hl_to_highlight_au(adjusted_highlight_name, ref_highlight, opts)
	vim.api.nvim_create_autocmd("ColorScheme", {
		pattern = "*",
		callback = function()
			match_hl_to_highlight(adjusted_highlight_name, ref_highlight, opts)
		end,
		group = theme_change_au,
	})
end
...
M.match_hl_to_highlight = match_hl_to_highlight_au -- link with adjusting color
M.highlight_from_src = highlight_from_src_au -- create new highlight from source highlight, and adjust color opt
